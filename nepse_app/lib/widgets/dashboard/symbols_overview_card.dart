...
class _SymbolRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final lastPrice = symbol.recentPrices.isEmpty ? null : symbol.recentPrices.first;
        ...
        children: [
          Row(
            children: [
              ...
              if (symbol.inPortfolio && symbol.inWatchlist) const SizedBox(width: 8),
          ...
          Wrap(
            ...
            children: [
              _InlineMetric(
                label: 'Shares',
                value: holding == null ? 'Not held' : holding.shares.toStringAsFixed(2),
              ),
              _InlineMetric(
                label: 'Current',
                value: holding?.currentPrice == null
                    ? (lastPrice == null ? 'No price' : currency.format(lastPrice.close))
                    : currency.format(holding!.currentPrice!),
              ),
              _InlineMetric(
                label: 'Last close',
                value: lastPrice == null ? 'No data' : currency.format(lastPrice.close),
              ),
              _InlineMetric(
                label: 'Snapshots',
